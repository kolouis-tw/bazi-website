...
本資料夾是 Louisko.com 首頁與「四柱八字十神排盤系統」子頁的前端網站工程包，不是大型多服務後端專案。主要可交付檔案是 `index.html`、`bazi.html` 與 `pages/` 內的預留次頁，其餘 Markdown 與 JSON 檔案用於交接、測試、版本紀錄與 AI Agent 維護。
...
- 八字十神排盤子頁：`bazi.html`
- 預留次頁：`pages/`
- 首頁/次頁工作流腳本：`scripts/site-workflow/manage-site.mjs`
- 首頁卡片設定：`scripts/site-workflow/site-pages.json`
- 專案說明：`README.md`
...
- `index.html` 是首頁入口，目前採無印良品風格：米白底、低飽和、留白、圓形入口按鈕。首頁應維持簡潔私人入口，不要改成行銷 landing page。
- `bazi.html` 是八字十神排盤工具頁，修改排盤邏輯時應只改此頁或同步更新相關文件。
- 未來新增一般次頁時，優先使用 `node scripts/site-workflow/manage-site.mjs add-page ...`，讓頁面放在 `pages/<slug>.html` 並自動更新首頁入口。
- 首頁工具卡片由 `scripts/site-workflow/site-pages.json` 與 `index.html` 中的 `LOUISKO_APP_CARDS_START` / `LOUISKO_APP_CARDS_END` marker 管理；不要刪除 marker。
- 不要在首頁或次頁加入提示詞、說明提醒、使用說明、安全提醒或教學文字，除非使用者明確要求顯示在頁面上。必要提醒可寫在 README / AGENTS 等內部文件，不放到公開頁面。
...
4. 若新增一般次頁，使用 `scripts/site-workflow/manage-site.mjs add-page`，再執行 `refresh-home` 或 `verify`。
5. 執行必要的本機預覽或人工測試。
6. 更新 README/changelog，記錄部署或行為改變。
7. 若要提交、推送與重新部署，可使用 `node scripts/site-workflow/manage-site.mjs publish --message "..." --zeabur`。
